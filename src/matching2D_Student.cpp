                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    {
         // OpenCV bug workaround : convert binary descriptors to floats
    ...
    { // k nearest neighbors (k=2)
        ...
        {
            if ((*it).at(0).distance < minRatio * (*it).at(1).distance)
            {
                matches.push_back((*it).at(0));
            }
        }
        std::cout << "Num keypoints removed = " << knn_matches.size() - matches.size() << std::endl;        
    }
}
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    {
        std::cout << "Invalid detector. Valid detector choices are: HARRIS, FAST, BRISK, ORB, AKAZE, SIFT " << std::endl;
        return;
    }
